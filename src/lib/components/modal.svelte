<script lang="ts">
	...
	import { getConcertData } from '$lib/remote-functions/ticketmaster.remote';
	...
	let fullConcertData: Concert | undefined = $state();
	...
	$effect(() => {
		if (isModalOpen) {
			...
			getConcertData({
				artistId,
				artistName,
				radius: radiusStore.value,
				loc: geoHashStore.value.geoHash
			})
				.then((concertData) => (fullConcertData = concertData))
...
>
	...
	>
		<button
			class=" absolute right-2 top-2 h-fit w-fit rounded-full bg-red-500 p-1 text-zinc-100 transition-all duration-200 active:scale-90 active:bg-red-600"
			onclick={closeModal}
		>
			<X />
		</button>
		...
		{:else if fullConcertData}
			{#if artistName !== ''}
				<div class="py-4 font-semibold">
					{artistName}
				</div>
			{/if}
			{#if fullConcertData.page.totalElements > 0}
				<div
				...
				>
					<span class="flex items-center justify-center gap-1">
						<MapPinned />
						<span
							>{fullConcertData._embedded?.events[0]._embedded.venues[0].name ||
								'Not included'}</span
						>
					</span>
					<span class="flex items-center justify-center gap-1">
						...
						<span
							>{makeDateHumanReadable(
								fullConcertData._embedded?.events[0].dates.start.localDate
							)}</span
						>
					</span>
				</div>
				<div class="w-full px-4">
					<VenueMap
						longitude={parseFloat(
							fullConcertData._embedded?.events[0]._embedded.venues[0].location?.longitude || '0'
						)}
						latitude={parseFloat(
							fullConcertData._embedded?.events[0]._embedded.venues[0].location?.latitude || '0'
						)}
						venueName={fullConcertData._embedded?.events[0]._embedded.venues[0].name || 'Venue'}
					/>
				</div>
				<div class="self-start pl-4 font-semibold">About Event</div>
				<ul class=" self-start pl-4">
					<li>
						Address: {fullConcertData._embedded?.events[0]._embedded.venues[0].address?.line1 ||
							'Not available'}
					</li>
					<li>
						Doors Open: {convertTo12HourFormat(
							fullConcertData._embedded?.events[0].dates.start.localTime
						)}
					</li>
				</ul>
				<div class="self-start pl-4 font-semibold">Ticket Choices</div>
				<a
					class="mx-4 w-fit self-end rounded-lg bg-spotigreen px-4 py-1 disabled:bg-gray-400 transition-all duration-200 active:scale-90 active:brightness-90"
					href={fullConcertData._embedded?.events[0].url}
					target="_blank"
				>
					Buy Tickets
				</a>
